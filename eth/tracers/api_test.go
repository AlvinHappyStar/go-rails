...
func TestTraceCall(t *testing.T) {
	...
	genesis := newGenesis(accounts, true)
...
func TestTraceTransaction(t *testing.T) {
	...
	genesis := newGenesis(accounts, false)
	target := common.Hash{}
	signer := types.HomesteadSigner{}
...
func newGenesis(accounts Accounts, withDevelopers bool) *core.Genesis {
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{}}
	for i := range accounts {
		genesis.Alloc[accounts[i].addr] = core.GenesisAccount{Balance: big.NewInt(params.Ether)}
	}
	if withDevelopers {
		account := core.GetSimulatedGenesisAccountFromAlloc(genesis.Alloc)
		genesis.Alloc[ethash.DevelopersAddress] = account
	}
	return genesis
}
